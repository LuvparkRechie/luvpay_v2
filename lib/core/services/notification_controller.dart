    ...
    await AwesomeNotifications().initialize(
      ...
      [
        NotificationChannel(
          channelKey: 'alerts',
          channelName: 'Alerts',
          channelDescription: 'General LuvPay alerts',
  ...
  static Future<void> scheduleNewNotification(int id, String title, String body,
    ...
    try {
      ...
      final tzz.TZDateTime scheduledTime = tzz.TZDateTime.from(
              DateTime.parse(dateSched!), tzz.getLocation('Asia/Manila'))
...
Future<void> getLogSession(context) async {
  ...
  final response = await Functions().requestHandler(
    apiKey: "${ApiKeys.getSession}?session_id=$sessionId",
  );
...
Future<void> getMessNotif() async {
  ...
  Functions()
      .requestHandler(apiKey: "${ApiKeys.getPaMessage}?user_id=$akongId")
      .then((messageData) async {
