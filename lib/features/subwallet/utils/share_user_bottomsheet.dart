...
class _ShareUserBottomSheetState extends State<ShareUserBottomSheet> {
  ...
  Future<void> _lookupUser(String mobile) async {
    ...
    try {
      ...
      final result = await Functions().requestHandler(apiKey: api);
            ...
            child: Column(
                ...
                children: [
                  ...
                  if (!isLoadingUser && displayName != null) ...[
                    const SizedBox(height: 10),
                    Row(children: [
                      Icon(isValidUser ? Icons.verified : Icons.error,
                          size: 16,
                      ...
                      Expanded(
                          child: LuvpayText(
                              ...
                              style: TextStyle(
                                  fontWeight: FontWeight.w900,
                                  color:
                                      isValidUser ? Colors.blue : Colors.red))),
                    ]),
                  ],
                  const SizedBox(height: 16),
                  LuvNeuPress.rectangle(
                      onTap: (canShare &&
                          ...
                          ? () async {
                              final normalized =
                                  normalizeMobile(controller.text);

                              widget.onSelected(
                                  normalized!, displayName ?? "", isValidUser);
