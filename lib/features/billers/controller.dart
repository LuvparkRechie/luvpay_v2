...
class BillersController extends GetxController {
  ...
  Future<void> getTemplate(billerData) async {
    ...
    Functions()
        .requestHandler(
            apiKey: "${ApiKeys.getBillerTemp}?biller_id=$billerId",
            method: "GET")
        .then((response) async {
      Get.back();
      if (response == "No Internet") {
  ...
  Future<void> generateQr() async {
    ...
    if (response["response"] == "Success") {
      ...
      CustomDialogStack.showSuccess(
          Get.context!, "Success", "Qr successfully changed", leftText: "Done",
          () {
        Get.back();
      });
      return;
  ...
  void deleteFavoriteBiller(int billerId) async {
    ...
    CustomDialogStack.showConfirmation(Get.context!, "Delete Biller",
      ...
      Functions()
          .requestHandler(
              apiKey: ApiKeys.deleteFavBiller, method: "DELETE", body: params)
          .then((retDelete) {
        Get.back();
        if (retDelete == "No Internet") {
          ...
          CustomDialogStack.showSuccess(
              Get.context!, "Success", "Successfully deleted", leftText: "Okay",
              () {
            Get.back();
            loadFavoritesAndBillers();
          });
